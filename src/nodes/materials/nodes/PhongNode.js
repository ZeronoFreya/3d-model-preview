...
PhongNode.prototype.build = function(builder) {
    ...
    if (builder.isShader("vertex")) {
        var position = this.position
            ? this.position.parseAndBuildCode(builder, "v3", {
                  cache: "position",
              })
            : undefined;
        ...
        if (this.environment)
            this.environment.parse(builder, { slot: "environment" });
        if (this.environmentAlpha && this.environment)
            this.environmentAlpha.parse(builder);
        ...
        var normal = this.normal
            ? this.normal.buildCode(builder, "v3")
            : undefined;

        var light = this.light
            ? this.light.buildCode(builder, "v3", { cache: "light" })
            : undefined;

        var ao = this.ao ? this.ao.buildCode(builder, "f") : undefined;
        var ambient = this.ambient
            ? this.ambient.buildCode(builder, "c")
            : undefined;
        var shadow = this.shadow
            ? this.shadow.buildCode(builder, "c")
            : undefined;
        var emissive = this.emissive
            ? this.emissive.buildCode(builder, "c", { slot: "emissive" })
            : undefined;

        var environment = this.environment
            ? this.environment.buildCode(builder, "c", { slot: "environment" })
            : undefined;
        var environmentAlpha =
            this.environmentAlpha && this.environment
                ? this.environmentAlpha.buildCode(builder, "f")
                : undefined;
	...
	*/
        if (alpha) {
            output.push(
                "gl_FragColor = vec4( outgoingLight, " + alpha.result + " );",
            );
        } else {
